...
object DailyRevenue {
  ...
  def main(args:Array[String]): Unit={
    ...
    val orderFiltered = orders.filter(rec=> rec.split(",")(1)=="MALIGN")
    val ordersMap = orderFiltered.map(rec => (rec.split(",")(0), rec.split(",")(1), rec.split(",")(2),rec.split(",")(3)))
    val order_item = sc.textFile(inputBaseDir + "/medical_prescribed")
    val orderFiltered1 = order_item.filter(rec=> rec.split(",")(3)=="MIT")
    val order_itemMap = orderFiltered1.map(rec => (rec.split(",")(0), rec.split(",")(1),rec.split(",")(2),rec.split(",")(3),rec.split(",")(4)))
    val orders_join = ordersMap.join(order_itemMap)
